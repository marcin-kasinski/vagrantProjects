...
def push(**kwargs):
    ...
    print("pushed_value_1 = {}".format(value_1))
...
def puller(**kwargs):
    ...
    print("pulled_value_1 = {}".format(pulled_value_1))
...
finisher = BashOperator(
    task_id="finish_task",
    bash_command='echo "{{ ti.xcom_pull(key="k1") }}" "{{ ti.xcom_pull(key="k2") }}" "{{ ti.xcom_pull(key="value from pusher 1") }}"',
    dag=dag,
)
