...
#      curl https://raw.githubusercontent.com/marcin-kasinski/vagrantProjects/master/kubernetes/yml/kafkaspotify.yml | kubectl apply -f -     
      ...
      while ! kubectl get po -o wide | grep mysql | grep Running ; do   echo "waiting for mysql IP..." ; sleep 5 ; done
      ...
      while ! nc -z $MYSQLPODIP 3306; do   echo "waiting mysql to launch ..." ; sleep 5 ; done


      cat <<EOT >> microserviceinit.sql
...
EOF
      ...
      mysqlshow -h $MYSQLPODIP --user=root --password=secret mysql | grep -v Wildcard | grep -o test
         ...
         if [ $? -gt 0 ] ; then
          ...
          mysql -h $MYSQLPODIP  -uroot -psecret  --port 3306  mysql < microserviceinit.sql
        else
          echo "jest baza"
    	fi
